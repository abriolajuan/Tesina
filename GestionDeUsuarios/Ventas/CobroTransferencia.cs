...
{
    public partial class CobroTransferencia : Form
    {
        ...
        public CobroTransferencia(string numeroCelu)
        {
            InitializeComponent();
        ...
        private void CobroTransferencia_Load(object sender, EventArgs e)
        {
        ...
        private void button5_Click(object sender, EventArgs e)
        {
                if (comboBox1.SelectedIndex != -1)
                {
                    ...
                    while (registros.Read())
                    {
                        ...
                        label9.Text = registros["saldoacobrar"].ToString();
                    }
                    registros.Close();
                    conexion.Close();
                }
        ...
        private void button1_Click(object sender, EventArgs e) //GUARDAR COBRO
        {
            ...
            {
                Aviso m = new Aviso();
                m.label1.Text = "Debe seleccionar una entidad de origen";
                m.ShowDialog();
            }
            ...
            {
                Aviso m = new Aviso();
                m.label1.Text = "Debe ingresar un N° de cuenta origen";
                m.ShowDialog();
            }
            ...
            else if (radioButton2.Checked && textBox7.Text != "")
            {
                ...
                saldoacobrar = float.Parse(label9.Text);
        ...
        {
            txtCelular = textBox6.Text;
            totalVenta = label2.Text;
            ...
            {
                cobroNuevo = label9.Text;
                saldoNew = 0;
            }
            else if (radioButton2.Checked)
            {
                cobroNuevo = textBox7.Text;
                saldoNew = (float.Parse(label9.Text)) - (float.Parse(cobroNuevo));
            }

            saldoNuevo = saldoNew.ToString();

            if (checkBox1.Checked)
                System.Diagnostics.Process.Start("https://web.whatsapp.com/send?phone=54" + txtCelular + "&text=" + "%C2%A1Hola!%20En%20tu%20compra%20por%20$" + totalVenta + "%20del%20" + fechaVenta + "hs,%20hemos%20recibido%20tu%20pago%20con%20transferencia%20por%20$" + cobroNuevo + ",%20quedando%20un%20saldo%20de%20$" + saldoNuevo + "%20%F0%9F%A7%BE%20%C2%A1Muchas%20gracias!%20%F0%9F%98%83%20MTA25%20%F0%9F%A5%AA");
        }
        ...
        private void guardarDetalleDeMedio()
        {
            ...
            conexion.Open();
            string sql = "insert into DETALLEDEMEDIO (MEDIO_TR_ID, DETMEDIO_APELLIDOTIT, DETMEDIO_NOMBRETIT, DETMEDIO_CUENTADEST, ENTIDAD_ID, COBRO_VENTA_ID) values (@medio,@apellidotitular,@nombretitular,@cuentaorig, @entidad, @cobroventa)";
            SqlCommand comando = new SqlCommand(sql, conexion);
            comando.Parameters.Add("@cobroventa", SqlDbType.Int).Value = cobroVentaId;
            comando.Parameters.Add("@medio", SqlDbType.Int).Value = "4";
            ...
            comando.Parameters.Add("@cuentaorig", SqlDbType.VarChar).Value = textBox5.Text;
            comando.ExecuteNonQuery();
            conexion.Close();
        }
